(() => {
  ...
  window.copyAgentCommand = async (id, type) => {
    const uninstall = "sudo systemctl disable --now vps-pulse-agent 2>/dev/null || true; sudo rm -f /etc/systemd/system/vps-pulse-agent.service /usr/local/bin/vps-pulse-agent /etc/vps-pulse-agent.token; sudo systemctl daemon-reload; echo 'VPS Pulse Agent 已卸载'";
    const command = type === 'install' ? sessionStorage.getItem(`vpspulse:install:${id}`) : uninstall;
  ...
  window.removeVPS = async id => {
    ...
    await api(`/api/vps/${id}`,{method:'DELETE'}); sessionStorage.removeItem(`vpspulse:install:${id}`); data=await api('/api/vps'); render();
  };
  ...
  render = function () {
    ...
    $('cards').innerHTML = data.map(v => `<article class="server-row">
      <div class="server-title"><span class="dot ${v.online?'on':''}"></span><div><b>${clean(v.name)} ${v.maintenanceUntil&&new Date(v.maintenanceUntil)>new Date()?'<em class="maintenance-badge">维护中</em>':''}</b><small>${clean(v.IPv4||v.ipv4||'等待 IP')} · Agent ${clean(v.AgentVersion||'旧版')}</small></div></div>
      ...
      <div class="server-menu"><details data-vps-id="${v.id}"><summary>操作</summary><div class="menu-pop">
        <button onclick="edit('${v.id}')">服务器设置</button><button onclick="openHistory('${v.id}')">历史图表</button><button onclick="act('${v.id}','ping')">立即 Ping</button><button onclick="act('${v.id}','changeip')">更换 IP</button><button onclick="act('${v.id}','dns')">更新 DNS</button><button onclick="confirm('确认在线升级 Agent？')&&act('${v.id}','upgrade-agent')">升级 Agent</button><button onclick="confirm('确认回滚到上一版 Agent？')&&act('${v.id}','rollback-agent')">回滚 Agent</button><button onclick="copyAgentCommand('${v.id}','install')">复制安装 Agent</button><button onclick="copyAgentCommand('${v.id}','uninstall')">复制卸载 Agent</button><button onclick="confirm('确认重启？')&&act('${v.id}','reboot')">重启 VPS</button><button class="danger" onclick="removeVPS('${v.id}')">删除 VPS</button>
      </div></details></div>
