...
func (c *config) set(target, command string, enabled bool) bool {
	c.Lock()
	changed := c.initialized && c.autoRecovery != enabled
	c.pingTarget = target
...
func (a *agent) readCommands() {
	...
	for {
		...
		if typ == "config" {
			...
			if a.cfg.set(strings.TrimSpace(target), strings.TrimSpace(command), enabled) {
